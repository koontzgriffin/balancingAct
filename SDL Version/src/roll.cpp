...
class Ball {
    public:
        ...
        void handle_collision(Beam beam){
            float norm[2];
            beam.get_normal(norm);
            float mag = norm[0]*velocity[0] + norm[1]*velocity[1];
            float component[2];
            component[0] = (velocity[0] - mag*norm[0]);
            component[1] = (velocity[1] - mag*norm[1]);

            float bouncyness[2];
            bouncyness[0] = (velocity[0] - 2*mag*norm[0])*0.2;
            bouncyness[1] = (velocity[1] - 2*mag*norm[1])*0.2;

            velocity[0] = component[0] + bouncyness[0];
            velocity[1] = component[1] + bouncyness[1];
        }
...
// game objects
Beam beam = Beam(SCREEN_WIDTH/2, (SCREEN_HEIGHT*3)/4, 280, 0);
